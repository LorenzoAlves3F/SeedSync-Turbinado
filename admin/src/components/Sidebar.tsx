...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  const items = [
    { id: 'dashboard', name: 'Desempenho', icon: LayoutDashboard },
    { id: 'clients', name: 'Pipelines', icon: Zap },
    { id: 'logs', name: 'Histórico', icon: ListFilter },
  ];
    ...
    <div className="w-80 bg-[#0A0D12] text-slate-300 h-screen flex flex-col border-r border-[#151D2A] p-6 shadow-2xl">
      ...
      <div className="mb-12 flex items-center gap-4 group cursor-pointer" onClick={() => setActiveTab('dashboard')}>
        ...
        <div>
          ...
          <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-[0.2em] mt-1 ml-0.5">Alta Performance</p>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-10">
        <nav className="space-y-2">
          <span className="px-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] block mb-4">Monitor</span>
          {items.map((item) => (
            <button
              key={item.id}
        ...
        <nav className="space-y-2">
          <span className="px-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] block mb-4">Conta</span>
          ...
          <button className="w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-slate-500 hover:text-slate-200 hover:bg-slate-800/30 transition-all group">
             ...
             <span className="text-sm font-bold">Acesso à API</span>
          </button>
        </nav>
      </div>
